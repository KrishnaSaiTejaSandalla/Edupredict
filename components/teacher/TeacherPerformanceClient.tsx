...
export default function TeacherPerformanceClient({ performance }: Props) {
  const { kpis, teachingEffectiveness, classOutcomes, aiInsights } = performance;

  const hasNoData =
    (!kpis || (kpis.teacherRating === 0 && kpis.attendanceCompletionRate === 0 && kpis.gradingRate === 0 && kpis.studentSatisfaction === 0)) &&
    (!teachingEffectiveness || teachingEffectiveness.length === 0) &&
    (!classOutcomes || classOutcomes.length === 0);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-8">
      ...
      {hasNoData ? (
        <div className="rounded-2xl border-2 border-dashed border-border bg-card p-12 text-center flex flex-col items-center justify-center space-y-4 max-w-2xl mx-auto transition-colors duration-200">
          ...
          <div className="space-y-1">
            <h2 className="text-lg font-semibold text-foreground">No performance data available</h2>
            <p className="text-xs text-muted-foreground max-w-sm mx-auto">
              As you begin marking attendance, grading assignments, and receiving feedback, your performance analytics will appear here.
            </p>
          </div>
        </div>
      ) : (
