...
export default async function ResultsPage({ searchParams }: Props) {
  ...
  // Fallback exam query: Try (classId + subjectId + type) exact match.
  // If not found and type was provided, fallback to (classId + subjectId) only.
...
{/* Results Table - check for no students first */}
        ...
        <div className="rounded-2xl border border-dashed border-border bg-card/40 p-16 text-center shadow-sm max-w-xl mx-auto mt-8">
          ...
          <h3 className="mt-4 text-base font-bold text-foreground">
            {classId && subjectId ? "No Exam Found" : "No Exam Selected"}
          </h3>
          <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
            {classId && subjectId
              ? "There is no exam record matching the selected Class and Subject parameters. Please confirm the configurations or add a new exam."
              : "Select a class and subject from the filters above to retrieve current examination results, average parameters, and grade allocations."}
          </p>
        </div>
      )}
    </main>
