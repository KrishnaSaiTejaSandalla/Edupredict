...
export default function TeacherLeavesClient({ initialHistory }: Props) {
  const [history, setHistory] = useState<LeaveHistory[]>(initialHistory);
  ...
  const confirmDelete = async () => {
    ...
    startTransition(async () => {
      try {
        ...
        toast.success("Leave request cancelled and deleted.");
        await reloadData();
      } catch (err: any) {
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 text-sm text-secondary">
            Request personal/sick time-off, check approval status, and view your historical requests.
          </p>
        </div>
